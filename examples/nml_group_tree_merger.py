    ...
    nodes = list(
        map(
            lambda x: [
                ...
                x.position[1] * scale[1],
                x.position[2] * scale[2],
            ],
            tree.nodes,
        )
    )
...
def merge_trees_to_one_tree(trees: List[Tree], scale):
    ...
    while len(trees) > 1:
        ...
        for current_tree in trees[1:]:
            node_coordinates_from_current_tree = get_np_array_of_nodes_with_scale(
                current_tree, scale
            )
            for current_node_index in np.arange(len(merging_tree.nodes)):
                current_node_coordinates = (
                    np.array(merging_tree.nodes[current_node_index].position) * scale
                )
                distances = dist(
                    node_coordinates_from_current_tree, current_node_coordinates
                )
        ...
        min_distance_tree.edges.append(
            Edge(
                merging_tree.nodes[min_merging_tree_node_index].id,
...
def create_merged_nml(nml: NML, scale):
    ...
    for current_group in nml.groups:
        ...
        logger.info(
            "Starting to merge group with id: {} and name: {}".format(
...
def load_and_save_merged_nml_trees(source: str, destination: str, scale):
    ...
    assert os.path.exists(
        os.path.dirname(os.path.realpath(destination))
...
def make_argparser():
    ...
    parser = argparse.ArgumentParser(
        "Script to merge NML-trees with the same group-id into one tree."
    )
    parser.add_argument(
        "--source", required=True, help="Path to the NML-file", type=str
    )
    parser.add_argument(
        "--destination",
        required=True,
    ...
    parser.add_argument(
        "--scale", help="Scale of the dataset (e.g. 11.2,11.2,25)", default="1,1,1"
    )
